	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("arapid", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("arapid"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{ //nolint:dupl
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "arapid")
