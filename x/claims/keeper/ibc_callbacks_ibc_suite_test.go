...
func (suite *IBCTestingSuite) TestOnAcknowledgementPacketIBC() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("arapid", "100", sender, receiver, "")
			...
			coin := suite.chainA.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "arapid")
			suite.Require().Equal(sdk.NewCoin("arapid", sdk.NewInt(tc.expectedBalance)).String(), coin.String())
...
func (suite *IBCTestingSuite) TestOnRecvPacketIBC() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("arapid", sdk.NewInt(claimableAmount/4)))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("arapid", amt.Add(amt.QuoRaw(2))))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("arapid", amt.QuoRaw(2)))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("arapid", triggerAmt, sender, receiver, "")
			bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			packet := channeltypes.NewPacket(bz, 1, path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID, timeoutHeight, 0)
			...
			coin := suite.chainB.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, "arapid")
			suite.Require().Equal(coin.String(), sdk.NewCoin("arapid", sdk.NewInt(tc.expectedBalance)).String())
