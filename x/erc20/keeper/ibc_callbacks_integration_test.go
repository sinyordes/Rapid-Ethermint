...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.Arapiddenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    utils.BaseDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.Arapiddenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert arapid", func() {
			// Register 'arapid' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'arapid' thru IBC
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.EvmosChain.GetContext(), evmosMeta)
			s.Require().NoError(err)

			arapidInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send arapid from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			arapidAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			s.Require().Equal(arapidInitialBalance.Amount.Sub(math.NewInt(amount)).Sub(sendAndReceiveMsgFee), arapidAfterBalance.Amount)

			// check ibc arapid coins balance on Osmosis
			arapidIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.arapidIbcdenom)
			s.Require().Equal(amount, arapidIBCBalanceBefore.Amount.Int64())

			// 2. Send arapid IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Arapiddenomtrace.Path, teststypes.Arapiddenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.arapidIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc arapid coins balance on Osmosis - should be zero
			arapidIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.arapidIbcdenom)
			s.Require().Equal(int64(0), arapidIBCSenderFinalBalance.Amount.Int64())

			// check arapid balance after transfer - should be equal to initial balance
			arapidFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			totalFees := sendBackCoinsFee.Add(sendAndReceiveMsgFee)
			s.Require().Equal(arapidInitialBalance.Amount.Sub(totalFees), arapidFinalBalance.Amount)

			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.arapidIbcdenom)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialarapidBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// should trigger claims logic and send arapid coins from claims to receiver
			...
			receiverFinalarapidBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, utils.BaseDenom)

			s.Require().Equal(receiverInitialarapidBalance.Amount.Add(claimableAmount).Sub(sendBackCoinsFee), receiverFinalarapidBalance.Amount)
		})
	})
