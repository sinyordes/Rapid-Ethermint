...
## default port prefixes for rapidd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/rapidd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/rapidd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/rapidd add-genesis-account \
    "$("$PWD"/build/rapidd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000arapid,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/rapidd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/rapidd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/rapidd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/rapidd start --pruning=nothing --rpc.unsafe \
